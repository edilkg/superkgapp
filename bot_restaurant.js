...
module.exports = function setupRestaurantBot(restBot, courierBot, clientBot, supabase, ADMIN_GROUP_ID) {
    ...
    restBot.start(async (ctx) => {
        try {
            ...
            console.log(`[Ресторан] Команда /start от ${id}`);
            
            // Используем maybeSingle(), чтобы бот не зависал при пустой базе
            ...
            if (fetchError) {
                console.error("Ошибка поиска в БД:", fetchError);
                return ctx.reply(`❌ Ошибка базы данных: ${fetchError.message}`);
            }

            if (!rest) {
                console.log(`[Ресторан] Создаем новую запись для ${id}`);
                const { error: insertError } = await supabase
                    .from('restaurants')
                    .insert([{ id: id, step: 'ask_name', is_approved: false }]);
                
                if (insertError) {
                    console.error("Ошибка записи в БД:", insertError);
                    return ctx.reply(`❌ Ошибка создания профиля: ${insertError.message}`);
                }
            ...
            return ctx.reply(`✅ Личный кабинет ресторана "${rest.name}" активен!\nСюда будут приходить новые заказы от клиентов.`);
        } catch (err) {
            console.error("Критическая ошибка /start:", err);
            ctx.reply("⚠️ Внутренняя ошибка бота. Проверьте логи сервера.");
        }
    });
    ...
    restBot.action(/rest_accept_(.+)/, async (ctx) => {
        ...
        ctx.editMessageText(`👨‍🍳 Заказ #${String(orderId).slice(0,5)} в работе!\n\nКогда всё будет упаковано, нажмите кнопку ниже:`,
            Markup.inlineKeyboard([
